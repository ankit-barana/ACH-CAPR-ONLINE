...
class Trial extends Component {
  ...
  audioHelper(amp) {
    localStorage.setItem("session", amp)
  }
  ...
  playStimulus = () => {
    ...
    //Send correct stimuli, uncomment stimuli that matches your test
    this.audioHelper(amp)
    // this.visualHelper(contrast)

    if (this.taskHelper()) {
      localStorage.setItem("type", 3);
    } else if (localStorage.getItem("testType") === "choice") {
      localStorage.setItem("type", 4)
    }else{
      this.props.shouldRecordRatings
